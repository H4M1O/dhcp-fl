...
# Script created by Claudio Proietti v. 1.0 created on 16/02/2016
...
# This is the main function that show the menu and allows the user to make a choice
	...
	do
		...
		# error handling loop, it checks if the user insert something different from a number between 1 and 6
		until [ $OPT -ge 1 -a $OPT -le 6 ] 
		do
			clear
		...
		# this infinite loop take the option selected by the user and call the relative function
...
# normally the only files contained in that directory should be
